...
class vkTools:
    ...
    def get_profile_info(self, user_id):
        ...
        result = {'name': (info['first_name'] + ' ' + info['last_name']) if 
                  ...
                  'city': info.get('city')['title'] if info.get('city') is not None else None if 'city' in info else None,
                  'bdate': self._bdate_toyear(info.get('bdate') if 'bdate' in info else None) 
                #   if None in  {'bdate','city', 'sex'}
                  }
        # print('есть None')
        return result

    def get_error_one (params,event,self,info):
        if  info.get('sex') == None:
            self.message_send(event.user_id, 'Не хватает информации напишите свой пол М или Ж')
